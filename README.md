...
Опубликованная версия: [hungryarthur.github.io](https://hungryarthur.github.io/).
...
При каждом обновлении ветки `main` GitHub Actions собирает статическую копию вики и
публикует её на `https://hungryarthur.github.io/`. Node.js-сервер на GitHub Pages не
нужен: страницы и данные API создаются заранее из снимка `game-data`.

Проверить ту же сборку локально:
...
После этого сайт доступен на `http://127.0.0.1:4173/`. Папка `dist` является
результатом сборки и не добавляется в Git — workflow создаёт её заново перед
публикацией.
