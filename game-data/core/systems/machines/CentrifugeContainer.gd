...
class_name CentrifugeContainer
...
const RECIPES: Dictionary = {
	...
	"gold_ore":     {"output": "gold_dust",     "byproduct": "copper_dust",   "output_count": 2},
	"silver_ore":   {"output": "silver_dust",   "byproduct": "gold_dust",     "output_count": 2},
	"titanium_ore": {"output": "titanium_dust", "byproduct": "iron_dust",     "output_count": 2},
...
func tick(delta: float) -> void:
	...
	var can_run: bool = (
		...
		and _slot_fits(slots[SLOT_OUTPUT], str(recipe.get("output", "")), int(recipe.get("output_count", 1)))
...
func _complete_process(recipe: Dictionary) -> void:
	...
	_add_to_slot(SLOT_OUTPUT, str(recipe.get("output", "")), int(recipe.get("output_count", 1)))
...
func _add_to_slot(slot_idx: int, item_id: String, count: int) -> void:
	if not ItemDatabase.registry.has(item_id):
		push_error("CentrifugeContainer: unknown item '%s'" % item_id)
		return
